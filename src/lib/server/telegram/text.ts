...
import { findUnfulfilled, getCodes, getUnclosed, orderButtons, unsufficientOrders } from "../queries/bot.js"
import { getYDBTimestamp, isEmpty, stringsFromQuery } from '../ydb/util.js'
import { REPLY } from './util.js'

export const simpleReplies = async (text: string, ctx: Context) => {
    switch(text) {
        case '/version':
            await ctx.reply('0.1.0')
            return REPLY
        ...
        default: throw 'unknown text in bot'
